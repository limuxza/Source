...
function Linux.Create(config)
    ...
    local ToggleHolder = Linux.Instance("ScreenGui", {
        Name = "LinuxToggleHolder",
        Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or game:GetService("CoreGui"),
        ResetOnSpawn = false,
        Enabled = true,
        ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    })
    ProtectGui(ToggleHolder)
    ...
    local function UpdateToggleVisibility()
        ...
        if not hasMoved then
            ...
            TweenService:Create(ToggleButton, tweenInfo, {Size = UDim2.new(0, 45, 0, 45)}):Play()
            task.wait(0.1)
            TweenService:Create(ToggleButton, tweenInfo, {Size = UDim2.new(0, 50, 0, 50)}):Play()
        end
    end
    ...
    ToggleButton.InputChanged:Connect(function(input)
        if isDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            if math.abs(delta.X) > dragThreshold or math.abs(delta.Y) > dragThreshold then
                ...
                local newX = math.clamp(startPos.X.Offset + delta.X, 0, ToggleHolder.AbsoluteSize.X - 50)
                local newY = math.clamp(startPos.Y.Offset + delta.Y, 0, ToggleHolder.AbsoluteSize.Y - 50)
    ...
    function LinuxLib.Destroy()
        ...
        for _, v in pairs(game:GetService("CoreGui"):GetChildren()) do
            if v:IsA("ScreenGui") and v.Name == "LinuxToggleHolder" then
                v:Destroy()
            end
        end
    end
